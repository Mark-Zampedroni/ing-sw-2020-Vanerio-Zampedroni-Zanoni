 ...
 * a string to transmit with {@link it.polimi.ingsw.network.messages.Message message} to the client and back
 *
 */
public class ModelSerialized implements Serializable {
    ...
    public String workersSerialized(int playerIndex) {
        List<Worker> workerArrayList = Session.getInstance().getPlayers().get(playerIndex).getWorkers();
